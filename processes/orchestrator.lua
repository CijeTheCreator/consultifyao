...
USDToken = USDToken or "FBt9A5GA_KXMMSxA2DJ0xZbAq8sLLU2ak-YJe9zDvg8"
...
Handlers.add("InstantiateUSDA", { Action = "InstantiateUSDA" }, function(msg)
	...
	if not msg.Tags.ProcessId then
		msg.reply({ Data = "No USDA token present" })
		return
	end
	USDToken = msg.Tags.ProcessId
	msg.reply({ Data = "USDA token instantiated successfully " .. USDToken })
end)
...
Handlers.add("Create", { Action = "Create" }, function(msg)
	...
	local tags = {
		PatientID = Patients[msg.From].patient_id,
		PatientLanguage = Patients[msg.From].language_preference,
	}
	-- Get notification that code is loaded
